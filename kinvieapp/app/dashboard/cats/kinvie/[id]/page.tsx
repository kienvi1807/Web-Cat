...
export default function CatDetailPage() {
  ...
  // 🎯 STATE MÀU SẮC & GIAO DIỆN SỔ XUỐNG
  ...
  const [isEmsOpen, setIsEmsOpen] = useState<boolean>(false); // ĐÓNG/MỞ BẢNG MÀU
  ...
  // 🎯 HÀM DỊCH MÃ EMS (Dùng cho Cây Phả Hệ)
    ...
    const initData = async () => {
      ...
      if (catId) {
        ...
        if (data) {
          ...
          setGender(data.gender !== false); // Load Giới tính

          // 🎯 TỰ ĐỘNG DỊCH NGƯỢC MÃ MÀU EMS ĐỂ CHỌN SẴN NÚT TRÊN UI
          ...
          // 🎯 TỰ ĐỘNG MAP TRẠI ĐỂ HIỆN MÈO BỐ/MẸ RA (ĐÃ SỬA LỖI)
          if (data.father_id && allCats) {
            const father = allCats.find(c => c.id === data.father_id);
            if (father) setFatherBreederId(father.breeder_id?.toString() || '1'); // Gán tạm trại số 1 nếu mất trại
          }
  ...
  // 🎯 HÀM TÌM CÁC "BẠN TÌNH" (CO-PARENTS) ĐÃ CÓ CON CHUNG TRONG DATABASE
  ...
  // 1. Chặn bản thân và hậu duệ của bản thân
  let baseExcluded = [currentCatId, ...getDescendants(currentCatId, allCatsList)];

  // 2. Chặn các "bạn tình" (đã có con chung) và cả tông ti họ hàng (tổ tiên, hậu duệ) của bạn tình
  ...
  // 3. Chặn chéo Bố/Mẹ đang được chọn trên Form
    ...
    <div className="animate-fade-in max-w-[1400px] mx-auto pb-24 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-10 gap-6 backdrop-blur-md bg-white/40 p-6 rounded-[2rem] border border-white/60 shadow-sm sticky top-4 z-50">
        <div className="flex items-center gap-6 w-full md:w-auto">
          <Link href="/dashboard/cats/kinvie" className="cursor-pointer w-12 h-12 flex items-center justify-center bg-white rounded-2xl shadow-sm text-stone-500 hover:text-orange-500 hover:shadow-orange-500/20 transition-all font-bold text-xl hover:-translate-x-1">←</Link>
          <div>
            <p className="text-xs font-black text-stone-400 uppercase tracking-widest mb-1">Hồ sơ Cattery / #{catId}</p>
            <h1 className="text-3xl font-sans font-black text-stone-800 flex items-center gap-3">{catData.name || 'Mèo Vô Danh'}</h1>
          </div>
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <button onClick={handleDeleteCat} disabled={isSaving || isUploading} className="cursor-pointer px-6 py-3 rounded-xl font-bold text-rose-500 hover:bg-rose-50 border border-transparent hover:border-rose-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Xóa Hồ Sơ</button>
          <button onClick={handleUpdateCat} disabled={isSaving || isUploading} className="cursor-pointer bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-black px-10 py-4 rounded-xl shadow-[0_4px_20px_rgba(249,115,22,0.4)] hover:shadow-[0_4px_30px_rgba(249,115,22,0.6)] transition-all transform hover:-translate-y-1 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          <div className="bg-white/60 backdrop-blur-2xl rounded-[2.5rem] p-8 lg:p-12 border border-white/80 shadow-[0_10px_50px_rgba(0,0,0,0.03)] relative overflow-hidden">
            ...
            <div className="space-y-8 relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* 🎯 BỔ SUNG NÚT CHỌN GIỚI TÍNH VÀO LẠI BẢN EDIT */}
                <div>
              ...
              {/* KHỐI CHỌN BỐ MẸ (ĐÃ FIX LỖI ẨN KHI CÓ DATA CŨ & CHẶN CẬN HUYẾT) */}
              {isPurebred && (
                <div className="bg-blue-50/50 rounded-3xl p-6 border border-blue-100 shadow-sm mt-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <select value={catData.father_id || ''} onChange={(e) => setCatData({...catData, father_id: e.target.value ? parseInt(e.target.value) : null})} disabled={!fatherBreederId} className="w-full bg-white border border-blue-200 px-4 py-3 rounded-xl text-sm font-bold shadow-sm outline-none appearance-none cursor-pointer disabled:bg-stone-100 disabled:text-stone-400">
                        <option value="">-- Chọn Mèo Bố --</option>
                        {allCatsList.filter(c => c.gender !== false && !excludedForFather.includes(c.id) && (c.breeder_id?.toString() === fatherBreederId || c.id === catData.father_id)).map(c => <option key={c.id} value={c.id}>♂ {c.name}</option>)}
                      </select>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={motherBreederId} onChange={(e) => { setMotherBreederId(e.target.value); setCatData({...catData, mother_id: null}); }} className="w-full bg-white border border-rose-200 px-4 py-3 rounded-xl text-sm font-bold shadow-sm outline-none appearance-none cursor-pointer">
                        <option value="">-- Chọn Trại giống --</option>
                        {breedersList.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
                      </select>
                    </div>
                    <div>
                      ...
                      <select value={catData.mother_id || ''} onChange={(e) => setCatData({...catData, mother_id: e.target.value ? parseInt(e.target.value) : null})} disabled={!motherBreederId} className="w-full bg-white border border-rose-200 px-4 py-3 rounded-xl text-sm font-bold shadow-sm outline-none appearance-none cursor-pointer disabled:bg-stone-100 disabled:text-stone-400">
                        <option value="">-- Chọn Mèo Mẹ --</option>
                        {allCatsList.filter(c => c.gender === false && !excludedForMother.includes(c.id) && (c.breeder_id?.toString() === motherBreederId || c.id === catData.mother_id)).map(c => <option key={c.id} value={c.id}>♀ {c.name}</option>)}
                      </select>
                    </div>
                  </div>
                </div>
              ...
              {isPurebred ? (
                <div className="bg-orange-50/50 rounded-3xl p-6 border border-orange-100 shadow-sm mt-8 transition-all duration-300">
                   ...
                   {/* Nội dung bảng màu chỉ hiện khi isEmsOpen là true */}
                ...
                <div className="bg-stone-50/50 rounded-3xl p-6 border border-stone-200 shadow-sm mt-8">
                  <div className="flex items-center justify-between mb-4">
                     ...
                     <div className="text-right">
                       <p className="text-lg font-black text-stone-600 bg-white px-3 py-1 rounded-lg border border-stone-200 shadow-sm">{simpleColor || catData.color || '???'}</p>
                     </div>
                  </div>
      ...
      {(catData.father_id || catData.mother_id) && (
        <div className="w-full bg-white/60 backdrop-blur-2xl rounded-[2.5rem] p-8 lg:p-12 border border-white/80 shadow-[0_10px_50px_rgba(0,0,0,0.03)] relative overflow-hidden mt-8 z-10">
          <div className="absolute inset-0 opacity-[0.02] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#f97316 1px, transparent 1px)', backgroundSize: '30px 30px' }}></div>
