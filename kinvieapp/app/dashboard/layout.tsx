...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        // 1. Thêm biến error vào để hứng lỗi từ Supabase
        const { data: { user }, error } = await supabase.auth.getUser();

        // 2. Nếu có lỗi (bao gồm lỗi Invalid Refresh Token) hoặc không có user
        if (error || !user) {
          console.warn("Auth Error/No User:", error?.message);
          await supabase.auth.signOut(); // Ép xóa sạch token rác ở client
        ...
        // 3. Fallback cuối cùng nếu toàn bộ khối try/catch bị sập
