  ...
  return (
    <div className="space-y-10 animate-fade-in max-w-4xl mx-auto pb-16">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-stone-500 mt-2 sm:mt-3 text-sm sm:text-lg">Kiểm soát hàng hóa, phụ kiện và theo dõi hạn sử dụng Pate tươi.</p>
      </div>
      ...
      <div className="relative group/section">
        ...
        <div className="relative bg-white/60 backdrop-blur-2xl rounded-[1.75rem] sm:rounded-[2.5rem] p-4 sm:p-8 md:p-10 border border-white/80 shadow-[0_8px_30px_rgba(0,0,0,0.03)] overflow-hidden transition-all duration-500 hover:border-slate-200/80 hover:shadow-[0_8px_50px_rgba(236,72,153,0.1)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8 relative z-10">
            ...
            {petshopModules.map((item) => (
              <Link href={item.path} key={item.name} className="relative group block h-full">
                ...
                <div className="relative h-full bg-white/90 backdrop-blur-sm rounded-2xl sm:rounded-3xl p-4 sm:p-8 flex flex-col items-center text-center z-10 shadow-[0_8px_20px_rgb(0,0,0,0.02)] border border-white">
                  
                  {/* Icon */}
                  <div className="text-4xl sm:text-6xl mb-2 sm:mb-6 group-hover:scale-110 transition-transform duration-500 drop-shadow-sm">
                    {item.icon}
                  </div>
                  
                  {/* Tiêu đề */}
                  <h3 className={`text-base sm:text-xl font-black text-${item.color}-600 mb-1.5 sm:mb-3 tracking-wide`}>
                    {item.name}
                  </h3>
                  
                  {/* Mô tả */}
                  <p className="text-sm text-stone-500 mb-4 sm:mb-8 flex-1 leading-relaxed px-2">
                    {item.description}
                  </p>
                  
                  {/* Khu vực Nhãn (Badge) Căn Giữa - Xóa Nút Bấm */}
                  <div className="w-full flex justify-center items-center mt-auto pt-3 sm:pt-6 border-t border-stone-100/80">
                    <span className={`${item.labelColor} border font-black px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-[11px] uppercase tracking-widest shadow-sm group-hover:scale-105 transition-transform duration-300`}>
                      {item.labelText}
                    </span>
                  </div>
