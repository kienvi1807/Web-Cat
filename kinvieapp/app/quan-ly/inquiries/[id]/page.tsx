...
// 🎯 TRANG NÀY DÀNH CHO STAFF (type_id=2) VÀ BREEDER (type_id=3), NẰM NGOÀI /dashboard
// Boss vẫn dùng trang cũ /dashboard/cats/inquiries/[id]
export default function StaffBreederInquiryDetailPage() {
  ...
  // 'staff' | 'breeder' | null -> null nghĩa là không có quyền
  const [accessLevel, setAccessLevel] = useState<'staff' | 'breeder' | null>(null);
  ...
  useEffect(() => {
    const checkAccessAndLoad = async () => {
      ...
      // 🎯 CHỈ STAFF (2) VÀ BREEDER (3) MỚI ĐƯỢC VÀO. BOSS/KHÁCH THƯỜNG -> CHẶN.
      ...
      // 🎯 BREEDER CHỈ ĐƯỢC XEM YÊU CẦU CỦA MÈO THUỘC CHÍNH TRẠI MÌNH
      ...
      setAccessLevel(dbUser.type_id === 2 ? 'staff' : 'breeder');
  ...
  if (accessDenied) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-stone-500">
        ...
        <p className="text-sm text-stone-400 mt-2">Chỉ Nhân viên hoặc Trại đối tác liên quan mới được xem yêu cầu này.</p>
        <Link href="/" className="mt-6 px-6 py-3 bg-pink-500 text-white rounded-full font-bold hover:bg-pink-600 transition-colors">Về trang chủ</Link>
      </div>
    );
  }
  ...
  // 🎯 Trang này ngoài dashboard nên trỏ ra trang public của mèo, không trỏ vào /dashboard/cats/... (staff/breeder không vào được dashboard)
  const catPublicLink = cat ? `/cattery/${cat.id}` : null;
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] pb-24 relative overflow-hidden selection:bg-pink-200">
      ...
      <div className="max-w-3xl mx-auto px-6 pt-12 relative z-10">
        <Link href="/" className="cursor-pointer group inline-flex items-center gap-2 bg-white/60 backdrop-blur-md border border-white text-pink-600 hover:bg-white px-5 py-2.5 rounded-full font-black text-sm mb-8 transition-all w-fit">
          <span className="transition-transform group-hover:-translate-x-1">←</span> Quay lại trang chủ
        </Link>
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-white rounded-[2.5rem] p-6 md:p-10 shadow-sm">
          ...
          {/* THÔNG TIN BÉ MÈO */}
          {cat && (
            <Link href={catPublicLink || '#'} className="flex items-center gap-4 bg-pink-50/50 border border-pink-100 rounded-2xl p-4 mb-8 hover:bg-pink-50 transition-colors group">
              <img
