...
export default function PetDetailPage() {
  ...
  const fetchPetData = async () => {
    ...
    if (petData.father_id) {
      const { data } = await supabase.from('pets').select('petid, petname, imageurl').eq('petid', petData.father_id).single();
      fatherData = data;
    }
    if (petData.mother_id) {
      const { data } = await supabase.from('pets').select('petid, petname, imageurl').eq('petid', petData.mother_id).single();
      motherData = data;
    }
